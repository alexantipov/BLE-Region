...
public struct QorumLogs {
    ...
    public static func test() {
        ...
        minimumLogLevelShown = 1
        QL_LOG_DEBUG(kLogDebug)
        QL_LOG_INFO(kLogInfo)
        QL_LOG_WARNING(kLogWarning)
        QL_LOG_ERROR(kLogError)
...
public func QL_LOG_DEBUG<T>(_ debug: T, _ file: String = #file, _ function: String = #function, _ line: Int = #line) {
    QLManager(debug, file: file, function: function, line: line, level:1)
}

///General information about app state
public func QL_LOG_INFO<T>(_ info: T, _ file: String = #file, _ function: String = #function, _ line: Int = #line) {
    QLManager(info,file: file,function: function,line: line,level:2)
}

///Indicates possible error
public func QL_LOG_WARNING<T>(_ warning: T, _ file: String = #file, _ function: String = #function, _ line: Int = #line) {
    QLManager(warning,file: file,function: function,line: line,level:3)
}

///An unexpected error occured
public func QL_LOG_ERROR<T>(_ error: T, _ file: String = #file, _ function: String = #function, _ line: Int = #line) {
...
private func printLog<T>( informationPart: String, text: T, level: Int) {
    ...
    var informationPart = informationPart
    let todaysDate:Date = Date()
    let dateFormatter:DateFormatter = DateFormatter()
    dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss.SSS"
    let currentTime:String = dateFormatter.string(from: todaysDate)
    
    informationPart = currentTime + " - " + informationPart
...
private func QLManager<T>(_ debug: T, file: String, function: String, line: Int, level : Int) {
    ...
    if QorumLogs.shouldPrintLine(level: level, fileName: filename) {
        ...
        printLog(informationPart: informationPart, text: debug, level: level)
...
private func QLineManager(_ lineString : String, file: String, function: String, line: Int) {
    ...
    if QorumLogs.shouldPrintLine(level: 2, fileName: filename) {
        let informationPart: String
        informationPart = "\(filename).\(fileExtension):\(line) \(function):"
        printLog(informationPart: informationPart, text: lineString, level: 5)
    }
}
